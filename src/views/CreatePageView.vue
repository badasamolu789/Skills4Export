...
const currentStep = ref<'choose' | 'details'>('choose')
const selectedCategory = ref<PageCategory | ''>('')
...
const handleSelectCategory = (category: PageCategory) => {
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-semibold text-[var(--text-primary)]">
              {{ currentStep === 'choose' ? 'Choose a page type' : `Create ${selectedCategoryLabel} page` }}
            </h2>
            <p class="mt-1 text-sm text-[var(--text-secondary)]">
              {{ currentStep === 'choose'
                ? 'Select the page type you want to create before proceeding to the page setup form.'
                : 'Fill in the page details and we’ll take you to the page detail screen when you’re done.' }}
            </p>
          </div>
        </div>

        <div v-if="currentStep === 'choose'" class="mt-6 grid gap-6 sm:grid-cols-2">
          <button
            v-for="option in categoryOptions"
            :key="option.value"
            type="button"
            @click="handleSelectCategory(option.value)"
            class="rounded-[1.5rem] border p-6 text-left transition"
            :class="
              selectedCategory === option.value
                ? 'border-[var(--accent)] bg-[linear-gradient(135deg,rgba(66,63,151,0.12),rgba(255,255,255,0.98))] shadow-[var(--shadow-soft)]'
                : 'border-[color:var(--border-soft)] bg-[var(--surface-secondary)] hover:border-[var(--accent)]'
            "
          >
            <div class="flex items-start gap-3">
              <span class="inline-flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-[var(--surface-primary)] text-[var(--accent-strong)]">
                <component :is="option.icon" class="h-6 w-6" />
              </span>
              <div>
                <p class="text-lg font-semibold text-[var(--text-primary)]">{{ option.label }}</p>
        ...
        <form v-else class="mt-6 space-y-6" @submit.prevent="handleSubmit">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex flex-wrap items-center gap-3">
              <button
                type="button"
                @click="goBackToCategorySelection"
                class="inline-flex items-center justify-center rounded-[1rem] border border-[color:var(--border-soft)] bg-[var(--surface-secondary)] px-4 py-3 text-sm font-semibold text-[var(--text-primary)] transition hover:border-[var(--accent)]"
              >
                Back
              </button>
              <span class="rounded-full bg-[var(--surface-secondary)] px-3 py-2 text-sm text-[var(--text-secondary)]">
                {{ selectedCategoryLabel }} page
              </span>
            </div>
          ...
          <div class="flex flex-col gap-3 border-t border-[color:var(--border-soft)] pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-[var(--text-secondary)]">
              Please confirm your page details before creating your page.
            </p>
            <button
              type="submit"
